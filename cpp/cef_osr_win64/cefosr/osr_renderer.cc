...
void mat4x4_ortho(mat4x4_t& matrix, float left, float right, float bottom, float top, float near, float far) {
  // row 0
...
void OsrRenderer::Initialize() {
  ...
  if (IsTransparent()) {
    ...
    glClearColor(float(CefColorGetR(settings_.background_color)) / 255.0f,
                 ...
                 float(CefColorGetB(settings_.background_color)) / 255.0f, 1.0f);
    VERIFY_NO_ERROR;
  }
...
void OsrRenderer::OnPopupSize(CefRefPtr<CefBrowser> browser, const CefRect& rect) {
...
void OsrRenderer::OnPaint(CefRefPtr<CefBrowser> browser,
  ...
  if (type == PET_VIEW) {
    ...
    if (old_width != view_width_ || old_height != view_height_ ||
      ...
      VERIFY_NO_ERROR;
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, view_width_, view_height_, 0, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV,
                   buffer);
      VERIFY_NO_ERROR;
    } else {
      ...
      for (; i != dirtyRects.end(); ++i) {
        ...
        VERIFY_NO_ERROR;
        glTexSubImage2D(GL_TEXTURE_2D, 0, rect.x, rect.y, rect.width, rect.height, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV,
                        buffer);
        VERIFY_NO_ERROR;
      }
    }
