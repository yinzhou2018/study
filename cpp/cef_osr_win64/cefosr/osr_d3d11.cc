...
void SwapChain::resize(int width, int height) {
  ...
  if (FAILED(hr)) {
    LOG(ERROR) << "d3d11: Failed to resize swapchain (" << width << "x" << height << ")";
    return;
  }
...
Geometry::Geometry(D3D_PRIMITIVE_TOPOLOGY primitive, uint32_t vertices, uint32_t stride, ID3D11Buffer* buffer)
    : primitive_(primitive), vertices_(vertices), stride_(stride), buffer_(to_com_ptr(buffer)) {}
...
std::shared_ptr<Device> Device::create() {
  ...
  if (SUCCEEDED(hr)) {
    ...
    LOG(INFO) << "d3d11: Selected adapter " << dev->adapter_name() << " and feature level 0x" << std::setw(4)
              << std::hex << selected_level;
...
std::shared_ptr<Geometry> Device::create_quad(float x, float y, float width, float height, bool flip) {
  ...
  if (SUCCEEDED(hr)) {
    return std::make_shared<Geometry>(D3D_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP, 4,
...
std::shared_ptr<ID3DBlob> Device::compile_shader(const std::string& source_code,
  ...
  const auto fnc_compile = reinterpret_cast<PFN_D3DCOMPILE>(GetProcAddress(lib_compiler_, "D3DCompile"));
  if (!fnc_compile) {
    return nullptr;
  }
...
// flags |= D3DCOMPILE_AVOID_FLOW_CONTROL;
  ...
  const auto hr =
      fnc_compile(psrc, len, nullptr, nullptr, nullptr, entry_point.c_str(), model.c_str(), flags, 0, &blob, &blob_err);
...
std::shared_ptr<Effect> Device::create_effect(const std::string& vertex_code,
  ...
  if (vs_blob) {
    ...
    device_->CreateInputLayout(layout_desc, elements, vs_blob->GetBufferPointer(), vs_blob->GetBufferSize(), &layout);
  }
...
Layer::Layer(const std::shared_ptr<Device>& device, bool flip) : device_(device), flip_(flip) {
...
void Layer::render_texture(const std::shared_ptr<Context>& ctx, const std::shared_ptr<Texture2D>& texture) {
  if (!geometry_) {
    geometry_ = device_->create_quad(bounds_.x, bounds_.y, bounds_.width, bounds_.height, flip_);
  }
...
Composition::Composition(const std::shared_ptr<Device>& device, int width, int height)
    : width_(width), height_(height), device_(device) {
  fps_ = 0.0;
  time_ = 0.0;
  frame_ = 0;
  fps_start_ = osr_utils::GetTimeNow();
}
...
FrameBuffer::FrameBuffer(const std::shared_ptr<Device>& device) : device_(device) {}
