...
BOOL LoadD3D12Dll(BOOL Load) {
  ...
  *(FARPROC*)&D3D12.CreateRootSignatureDeserializer =
      GetProcAddress(hModuleD3D12, "D3D12CreateRootSignatureDeserializer");

  if (!D3D12.CreateDevice || !D3D12.GetDebugInterface || !D3D12.SerializeRootSignature ||
...
void DescriptorArray::Initialize(ID3D12Device* Device,
  ...
  mGpuBase = (HeapFlags & D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE) ? mHeap->GetGPUDescriptorHandleForHeapStart()
                                                                     : D3D12_GPU_DESCRIPTOR_HANDLE{NULL};
}

bool FrameQueue::Initialize(const char* DebugName,
                            ID3D12Device* Device,
                            ID3D12CommandQueue* CommandQueue,
                            ID3D12PipelineState* InitialPipelineState,
                            void* FrameUserData,
                            UINT SizeofStructFrame,
                            UINT FrameCount,
  ...
  for (UINT i = 0; i < FrameCount; ++i) {
    ...
    CheckHresult(
        Device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&Frame->mCommandAllocator)));
...
bool FrameQueue::SetSwapChain(IDXGISwapChain2* SwapChain, DXGI_FORMAT RenderTargetViewFormat, float dpiX, float dpiY) {
  if (!SwapChain) {
    mBackBuffers.clear();
    return true;
  }
  ...
  for (UINT i = 0; i < ChainLength; ++i) {
    ...
    if (mDevice11On12) {
      ...
      CheckHresult(mD2DDeviceContext->CreateBitmapFromDxgiSurface(surface.Get(), &bitmapProperties,
