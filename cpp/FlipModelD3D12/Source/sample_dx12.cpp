...
struct RootParameters {
  ...
  enum { DefaultSampler, StaticSamplerCount };
};
...
enum { PerVertexInputSlot, PerInstanceInputSlot };

struct cbuffer {
  float4x4 projection;

  cbuffer() { load_identity(&projection); }
};
...
static bool initialize_dx12_internal() {
  ...
  // Create the dxgi factory
  {
    ...
    if (use_debug_layer)
      dxgiFactory2Flags |= DXGI_CREATE_FACTORY_DEBUG;
  ...
  // init 11On12
  {
    CheckHresult(D3D11On12CreateDevice(device, d3d11DeviceFlags, nullptr, 0,
                                       reinterpret_cast<IUnknown**>(dx12->command_queue.GetAddressOf()), 1, 0,
                                       &dx12->device11, &dx12->device11context, nullptr));
...

  {
    ...
    ThrowIfFailed(DWriteFactory->CreateTextFormat(L"Segoe UI", NULL, DWRITE_FONT_WEIGHT_NORMAL,
  ...
  // Create the root signature
  {
    ...
    root_sig_desc.Init(_countof(parameters), parameters, 0, nullptr,
    ...
    CheckHresult(device->CreateRootSignature(0, pOutBlob->GetBufferPointer(), pOutBlob->GetBufferSize(),
  ...
  // Create the pipeline state
  {
    ...
    static const D3D12_INPUT_ELEMENT_DESC layout[] = {
        // per vertex data
        {"position", 0, DXGI_FORMAT_R32G32B32_FLOAT, PerVertexInputSlot, (UINT)offsetof(color_vertex, x),
         D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        {"color", 0, DXGI_FORMAT_R8G8B8A8_UNORM, PerVertexInputSlot, (UINT)offsetof(color_vertex, r),
         D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},

        // per instance data
        {"modelview", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, PerInstanceInputSlot,
         (UINT)offsetof(instance_data, modelview.m[0]), D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1},
        {"modelview", 1, DXGI_FORMAT_R32G32B32A32_FLOAT, PerInstanceInputSlot,
         (UINT)offsetof(instance_data, modelview.m[1]), D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1},
        {"modelview", 2, DXGI_FORMAT_R32G32B32A32_FLOAT, PerInstanceInputSlot,
         (UINT)offsetof(instance_data, modelview.m[2]), D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1},
        {"modelview", 3, DXGI_FORMAT_R32G32B32A32_FLOAT, PerInstanceInputSlot,
         (UINT)offsetof(instance_data, modelview.m[3]), D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1},
    };
  ...
  dx12->frame_q.Initialize("Frames", device, dx12->command_queue.Get(), dx12->perspective_pipeline.Get(),
  ...
  for (size_t i = 0; i < dx12->frames.size(); ++i) {
    ...
    CheckHresult(device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, ctx->mCommandAllocator.Get(),
                                           dx12->perspective_pipeline.Get(), IID_PPV_ARGS(&frame.mCommandList)));
    ...
    frame.eviz_vertices =
        MakeVertexBufferView(gpu_base, dynamic->eventviz_verts, sizeof(dynamic->eventviz_verts), dynamic);
  ...
  // Create and fill the geometry buffer & constant buffers
  {
    ...
    dx12->cube_vbuf =
        MakeVertexBufferView(gpu_base, constant_data->cube_vbuf, sizeof(constant_data->cube_vbuf), constant_data);
    dx12->cube_ibuf = MakeIndexBufferView(gpu_base, constant_data->cube_ibuf, sizeof(constant_data->cube_ibuf),
                                          constant_data, DXGI_FORMAT_R16_UINT);
...
static void present_dx12(frame_data* frame, UINT64 FrameBeginTime, int vsync, dx12_render_stats* out_stats) {
  ...
  auto present_entry =
      eviz->Start(EventViz::kPresentQueue, &event_types[EVENT_TYPE_COLOR0 + color_index], frame->render_id);
...
static bool resize_dx12_internal(void* pHWND, void* pCoreWindow, float x_dips, float y_dips, float dpi) {
  ...
  swap_chain_desc.Flags =
      swapchain_opts.create_time.use_waitable_object ? DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT : 0;
  ...
  if (!dx12->swap_chain) {
    ...
    CheckHresult(dxgi_factory->CreateSwapChainForHwnd(dx12->command_queue.Get(), hwnd, &swap_chain_desc, NULL, NULL,
                                                      &base_chain));
    ...
    ComPtr<IDXGISwapChain1> base_chain;
    CheckHresult(dxgi_factory->CreateSwapChainForCoreWindow(
        dx12->command_queue.Get(), reinterpret_cast<IUnknown*>(pCoreWindow), &swap_chain_desc, nullptr, &base_chain));
    CheckHresult(base_chain.As(&dx12->swap_chain));
  ...
  // Resize the existing swap chain (window resized)
  else if (dx12->swap_chain_width < dx12->screen_width || dx12->swap_chain_height < dx12->screen_height ||
    ...
    // wsi::log_message(0, 0, "Resizing buffers: %dx%d -> %dx%d", old_width, old_height, dx12->swap_chain_width,
    ...
    CheckHresult(dx12->swap_chain->ResizeBuffers(swapchain_opts.create_time.swapchain_buffer_count,
                                                 dx12->swap_chain_width, dx12->swap_chain_height,
                                                 swap_chain_desc.Format, swap_chain_desc.Flags));
  ...
  if (create_depth) {
    ...
    CheckHresult(device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
                                                 D3D12_HEAP_FLAG_NONE, &depth_desc, D3D12_RESOURCE_STATE_DEPTH_WRITE,
                                                 &clear_depth, IID_PPV_ARGS(&dx12->depth_buffer)));
    SetName(dx12->depth_buffer, "depth_buffer");

    device->CreateDepthStencilView(dx12->depth_buffer.Get(),
  ...
  load_simple_perspective(&dx12->perspective, 45.0f, viewport.Width / viewport.Height, 0.1f, 100.0f, viewport.MinDepth,
                          viewport.MaxDepth);
  load_simple_ortho(&dx12->ortho, dx12->screen_x_dips, dx12->screen_y_dips, 100.0f, viewport.MinDepth,
                    viewport.MaxDepth);
...
static UINT build_eviz_display(color_vertex (&write)[MAX_EVIZ_VERTS],
                               UINT* eviz_tri_start,
                               UINT* eviz_tri_count,
                               UINT* eviz_line_start,
                               UINT* eviz_line_count,
...
static void build_frame(ID3D12GraphicsCommandList* command_list,
  ...
  build_eviz_display(data->eventviz_verts, &eviz_tri_start, &eviz_tri_count, &eviz_line_start, &eviz_line_count, 16);
...
static void render_d2d(FrameQueue::FrameContext* ctx, const WCHAR* text) {
  ...
  d2dcontext->DrawText(text, (UINT32)wcslen(text), format, &textRect, brush);
...
void render_game_dx12(wchar_t* hud_text,
                      game_data* game,
                      float fractional_ticks,
                      int vsync_interval,
                      dx12_render_stats* stats) {
  auto CpuFrameStartFull = QpcNow();
  if (stats) {
  ...
  frame->timestamps.QueryTimestampCommand(command_list, (UINT64*)offsetof(frame_timestamps_struct, full[0]));

  {
    auto TransitionPresentToRenderTarget = CD3DX12_RESOURCE_BARRIER::Transition(
        ctx->mBackBuffer, D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
...

  {
    ...
    auto render_event =
        eviz->Start(EventViz::kCpuQueue, &event_types[EVENT_TYPE_COLOR0 + color_index], frame->render_id);
    auto command_list = frame->mCommandList.Get();
    ...
    frame->timestamps.QueryTimestampCommand(command_list, (UINT64*)offsetof(frame_timestamps_struct, full[1]));
  ...
  if (stats) {
    ...
    stats->gpu_frame_time =
        float(double(timestamps->draw[1] - timestamps->draw[0]) / dx12->CommandQueuePerformanceFrequency);
    stats->cpu_frame_time_full = float(double(CpuFrameEndFull - CpuFrameStartFull) / g_QpcFreq);
    stats->gpu_frame_time_full =
        float(double(timestamps->full[1] - timestamps->full[0]) / dx12->CommandQueuePerformanceFrequency);
  }
}

bool set_swapchain_options_dx12(void* pHWND,
                                void* pCoreWindow,
                                float x_dips,
                                float y_dips,
                                float dpi,
                                dx12_swapchain_options* opts) {
  bool recreate = false;
  ...
  if (!dx12) {
    ...
    if (!x_dips)
      x_dips = dx12->screen_x_dips;
    if (!y_dips)
      y_dips = dx12->screen_y_dips;

    if (dx12->device && FAILED(dx12->device->GetDeviceRemovedReason())) {
      recreate = true;
    } else if (memcmp(&swapchain_opts.create_time, &opts->create_time, sizeof(dx12_swapchain_options::create_time))) {
      recreate = true;
    }
  }
  ...
  } else if (x_dips != dx12->screen_x_dips || y_dips != dx12->screen_y_dips || dpi != dx12->swap_chain_dpi) {
    return resize_dx12_internal(pHWND, pCoreWindow, x_dips, y_dips, dpi);
  } else {
    memcpy(&swapchain_opts, opts, sizeof(dx12_swapchain_options));
