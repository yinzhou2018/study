...
- [x] 1.3 创建 quic-stream-api 模块（pom.xml + 空目录结构）
- [x] 1.4 创建 quic-stream-core 模块（pom.xml + 空目录结构）
- [x] 1.5 创建 quic-stream-example 模块（pom.xml + 空目录结构）
- [x] 1.6 生成自签名 TLS 证书（keytool 脚本，输出到 quic-stream-core/src/main/resources/certs/）
