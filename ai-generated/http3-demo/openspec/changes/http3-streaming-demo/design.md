...
本项目是一个全新的 HTTP/3 流式通信示例项目，零历史代码。项目需基于 Java 17 + Maven 构建，底层使用 Netty + `netty-incubator-codec-quic` 实现 HTTP/3 QUIC 协议栈，上层封装泛型化的双向流式消息通信 API。
...
- 基于 Netty QUIC 实现 HTTP/3 客户端和服务端
...
```
http3-demo (parent pom)
├── http3-api      — 纯接口模块，零依赖，可独立发布
├── http3-core     — 实现模块，依赖 http3-api + Netty
└── http3-example  — 示例模块，依赖 http3-core
```

### 6. 自签名 TLS 证书

- 使用 keytool 在构建时生成自签名证书
- QUIC 基于 TLS 1.3，HTTP/3 必须依赖 TLS
